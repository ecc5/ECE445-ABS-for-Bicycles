...
float IIR(float input, float old) {
	return (1-IIR_alpha)*old + input;
}

void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if (GPIO_Pin & (1<<1)) {
//		delta_t1 = HAL_GetTick() - delta_t1;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  //Also, you need to define the clock in the ioc.

	  //First get some way of reading the sensor input (requires a rising edge interrupt)
	  //also needs to find some way to get the difference between rising edges
	  //(maybe increment a counter every certain cpu cycles?) [might have error due to overflow]
	  //definitely make the counter reset after every rising edge event finishes

	  //Still need to define some way of getting the sensor input, but for now,
	  //Just know that you want some rad/delta_t as a way of finding immediate velocity
	  //Then send those into the filter like so vvv
